...
/// <summary>
/// A BELLATRIX FindStrategy for iOS that uses Semantic Kernel to locate elements by natural language prompts.
/// Primary resolution attempts to match known PageObject locators via Retrieval-Augmented Generation (RAG).
/// Fallback resolution builds a fresh NSPredicate string from the current view snapshot via prompt.
/// </summary>
public class FindByPrompt : FindStrategy<IOSDriver, AppiumElement>
{
    ...
    /// <summary>
    /// Locates a single AppiumElement using the resolved NSPredicate strategy.
    /// </summary>
    ...
    /// <summary>
    /// Locates all matching AppiumElements using the resolved NSPredicate strategy.
    /// </summary>
    ...
    /// <summary>
    /// Resolves the NSPredicate locator using RAG memory, cache, or fallback prompt regeneration.
    /// </summary>
    ...
    private string TryResolveFromPageObjectMemory(string instruction, object context)
    {
        ...
        return ParsePromptLocatorToNSPredicate(locatorResult);
    }
    ...
    /// <summary>
    /// Parses a prompt result string into an NSPredicate string (expects 'nspredicate=...').
    /// </summary>
    private static string ParsePromptLocatorToNSPredicate(string promptResult)
    {
        ...
        if (!parts.Success)
        {
            throw new ArgumentException($"❌ Invalid format. Expected: nspredicate=... but received '{promptResult}'");
        }
    ...
    /// <summary>
    /// Checks if any elements are present for the given NSPredicate in the provided context.
    /// </summary>
    private static bool IsElementPresent(object context, string nspredicate)
    {
        try
        {
            if (context is IOSDriver driver)
                return driver.FindElements(MobileBy.IosNSPredicate(nspredicate)).Any();
            if (context is AppiumElement element)
                return element.FindElements(MobileBy.IosNSPredicate(nspredicate)).Any();
            return false;
        }
        catch { return false; }
    }
