...
public static class CloudProviderCredentialsResolver
{
    ...
    {
        if (!ConfigurationService.GetSection<WebSettings>().ExecutionSettings.Arguments[0].ContainsKey(USER_ENVIRONMENTAL_VARIABLE) ||
            !ConfigurationService.GetSection<WebSettings>().ExecutionSettings.Arguments[0].ContainsKey(ACCESS_KEY_ENVIRONMENTAL_VARIABLE))
        {
        ...
        string user = ConfigurationService.GetSection<WebSettings>().ExecutionSettings.Arguments[0][USER_ENVIRONMENTAL_VARIABLE];
        string accessKey = ConfigurationService.GetSection<WebSettings>().ExecutionSettings.Arguments[0][ACCESS_KEY_ENVIRONMENTAL_VARIABLE];
