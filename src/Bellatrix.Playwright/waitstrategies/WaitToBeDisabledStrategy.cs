...
public class WaitToBeDisabledStrategy : WaitStrategy
{
    ...
    {
        Expect(by.Convert(WrappedBrowser.CurrentPage)).ToBeDisabledAsync(new() { Timeout = TimeoutInterval }).GetAwaiter().GetResult();
    }

    public override void WaitUntil<TBy>(TBy by, Component parent)
    {
        Expect(by.Convert(parent.WrappedElement)).ToBeDisabledAsync(new() { Timeout = TimeoutInterval }).GetAwaiter().GetResult();
    }
}