...
public class BrowserService : WebService
{
    ...
    {
        int maxSeconds = ConfigurationService.GetSection<WebSettings>().TimeoutSettings.InMilliseconds().WaitUntilReadyTimeout;
        ...
        Bellatrix.Utilities.Wait.ForConditionUntilTimeout(
                () =>
                {
                    ...
                    {
                        Logger.LogInformation($"Exception occurred while waiting for ReadyState = complete. Message: {ex.Message}");
                    }
    ...
    public void WaitForAngular()
    {
        ...
        string isAngular5 = InvokeScript("getAllAngularRooTComponents().attributes['ng-version']");
        if (!string.IsNullOrEmpty(isAngular5))
        {
            Bellatrix.Utilities.Wait.Until(() => bool.Parse(InvokeScript("window.getAllAngularTestabilities().findIndex(x=>!x.isStable()) === -1")), waitForAngularTimeout);
        }
        else
        {
            bool? isAngularDefined = bool.Parse(InvokeScript("window.angular === undefined").ToString());
            if (isAngularDefined == null || !((bool)isAngularDefined))
            {
                bool isAngularInjectorUnDefined = bool.Parse(InvokeScript("angular.element(document).injector() === undefined").ToString());
                if (!isAngularInjectorUnDefined)
                {
                    Bellatrix.Utilities.Wait.Until(() => bool.Parse(InvokeScript("angular.element(document).injector().get('$http').pendingRequests.length === 0")), waitForAngularTimeout);
                }
            }
        }
    }
    ...
    public void WaitForJavaScriptAnimations()
    {
        ...
        Bellatrix.Utilities.Wait.Until(
            () =>
            {
                if (bool.TryParse((string)InvokeScript("jQuery && jQuery(':animated').length === 0"), out bool result))
                {
                    return result;
                }
                else
            ...
            retryRateDelay: ConfigurationService.GetSection<WebSettings>().TimeoutSettings.SleepInterval);
    }
    ...
    public void WaitForAjaxRequest(string requestPartialUrl, int additionalTimeoutInSeconds = 0)
    {
        Bellatrix.Utilities.Wait.Until(
            () =>
            {
                string result = InvokeScript($"performance.getEntriesByType('resource').filter(item => item.initiatorType == 'xmlhttprequest' && item.name.toLowerCase().includes('{requestPartialUrl.ToLower()}'))[0] !== undefined;");
                if (result == "True")
                {
                    return true;
                }
    ...
    {
        int maxMilliseconds = ConfigurationService.GetSection<WebSettings>().TimeoutSettings.InMilliseconds().WaitForAjaxTimeout;
        ...
        Bellatrix.Utilities.Wait.ForConditionUntilTimeout(
            () =>
            {
                numberOfAjaxConnections = InvokeScript("!isNaN(window.openHTTPs) ? window.openHTTPs : null");
            ...
            totalRunTimeoutMilliseconds: maxMilliseconds,
            sleepTimeMilliseconds: 300,
            onTimeout: () => { Logger.LogWarning($"Timed out waiting for open connections to be closed. Wait time: {maxMilliseconds} sec."); });
    }
    ...
    {
        var numberOfAjaxConnections = InvokeScript("!isNaN(window.openHTTPs) ? window.openHTTPs : null");
