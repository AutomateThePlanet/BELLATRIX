...
using Bellatrix.Playwright.lighthouse;
...
public class LighthouseService
{
    ...
    public void PerformLighthouseAnalysis(string customArgs = "", bool shouldOverrideDefaultArgs = false)
    {
        ...
        {
            arguments = $"lighthouse {browserService.Url} --output=json,html,csv --port={WrappedBrowserCreateService.DebuggerPort} {customArgs}";
        }
        else
        {
            var defaultArgs = GetDefaultLighthouseArgs();
            arguments = $"lighthouse {browserService.Url} --output=json,html,csv --port={WrappedBrowserCreateService.DebuggerPort} {defaultArgs} {customArgs}";
        }

        var settings = ConfigurationService.GetSection<LighthouseSettings>();

        if (WrappedBrowserCreateService.BrowserConfiguration.ExecutionType == ExecutionType.Regular)
        {
