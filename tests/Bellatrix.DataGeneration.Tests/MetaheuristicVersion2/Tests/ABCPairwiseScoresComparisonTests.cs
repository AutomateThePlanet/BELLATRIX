...
namespace Bellatrix.Web.Tests.MetaheuristicVersion2.Tests
{
    ...
    public class ABCOptimizationBenchmarkTests
    {
        ...
        private void RunBenchmarkForParameterSet(ABCParameterSet paramSet)
        {
            ...
            {
                double abcTotalScore = RunSingleIteration(paramSet);
        ...
        private double RunSingleIteration(ABCParameterSet paramSet)
        {
            ...
            int topCount = (int)(_sortedPairwiseScores.Count * paramSet.SelectionRatio);
            double pairwiseTotalScore = _sortedPairwiseScores.Take(topCount).Sum(p => p.Value);
        ...
        {
            double avgAbcScore = _abcScores[paramSet].Average();
            double avgPairwiseScore = _pairwiseScores[paramSet].Average();
            double percentageImprovement = ((avgAbcScore - avgPairwiseScore) / avgPairwiseScore) * 100;
