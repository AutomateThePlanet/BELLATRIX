// <copyright file="ProgressControlTestsInternetExplorer.cs" company="Automate The Planet Ltd.">
// Copyright 2020 Automate The Planet Ltd.
// Licensed under the Apache License, Version 2.0 (the "License");
// You may not use this file except in compliance with the License.
// You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// </copyright>
// <author>Anton Angelov</author>
// <site>https://bellatrix.solutions/</site>
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace Bellatrix.Web.Tests.Controls
{
    [TestClass]
    [Browser(BrowserType.InternetExplorer, Lifecycle.ReuseIfStarted)]
    [AllureSuite("Progress Control")]
    public class ProgressControlTestsInternetExplorer : MSTest.WebTest
    {
        public override void TestInit() => App.NavigationService.NavigateToLocalPage(SettingsService.GetSection<TestPagesSettings>().ProgressLocalPage);

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void Return70_When_UseGetValueMethod_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress");

            Assert.AreEqual("70", progressElement.Value);
        }

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void ReturnNull_When_NoValueAttributeAttributePresent_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress2");

            Assert.IsNotNull(progressElement.Value);
        }

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void Return100_When_UseGetMaxMethod_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress");

            Assert.AreEqual("100", progressElement.Max);
        }

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void Return1_When_NoMaxAttributePresent_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress1");

            var actualMax = progressElement.Max;

            Assert.AreEqual("1", actualMax);
        }

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void Return70_When_UseGetInnerTextMethod_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress");

            Assert.AreEqual("70 %", progressElement.InnerText);
        }

        [TestMethod]
        [Ignore, TestCategory(Categories.InternetExplorer), TestCategory(Categories.Windows)]
        public void ReturnEmpty_When_NoInnerTextPresent_InternetExplorer()
        {
            var progressElement = App.ElementCreateService.CreateById<Progress>("myProgress3");

            var actualInnerText = progressElement.InnerText;

            Assert.AreEqual(string.Empty, actualInnerText);
        }
    }
}