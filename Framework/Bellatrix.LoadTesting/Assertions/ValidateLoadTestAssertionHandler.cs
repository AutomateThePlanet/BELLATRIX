// <copyright file="ValidatesLoadTestAssertionHandler.cs" company="Automate The Planet Ltd.">
...
{
    public class ValidatesLoadTestAssertionHandler : LoadTestAssertionHandler
    {
        ...
        public ValidatesLoadTestAssertionHandler(List<LoadTestLocator> loadTestLocators, List<LoadTestValidateHandler> loadTestValidateHandler)
        {
                ...
                foreach (var responseAssertion in httpRequestDto.ResponseAssertions)
                {
                    ...
                    if (_loadTestLocators.Any(x => x.LocatorType.Equals(responseAssertion.Locator)))
                    {
                        ...
                        try
                        {
                            ...
                            if (_loadTestValidateHandler.Any(x => x.ValidateType.Equals(responseAssertion.AssertionType)))
                            {
                                var currentValidateHandler = _loadTestValidateHandler.First(x => x.ValidateType.Equals(responseAssertion.AssertionType));
                                responseAssertionResults = currentValidateHandler.Execute(htmlElement, responseAssertion.ExpectedValue);
                            }
                            else
                            {
