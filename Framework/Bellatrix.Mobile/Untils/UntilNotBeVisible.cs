...
{
    public class UntilNotBeVisible<TDriver, TDriverElement> : BaseUntil<TDriver, TDriverElement>
       where TDriver : AppiumDriver<TDriverElement>
       where TDriverElement : AppiumWebElement
    {
        public UntilNotBeVisible(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetMobileSettings().ElementNotToBeVisibleTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementIsInvisible(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<IWebDriver, bool> ElementIsInvisible<TBy>(TDriver searchContext, TBy by)
