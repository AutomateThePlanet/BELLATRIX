...
{
    public class UntilExist<TDriver, TDriverElement> : BaseUntil<TDriver, TDriverElement>
       where TDriver : AppiumDriver<TDriverElement>
       where TDriverElement : AppiumWebElement
    {
        public UntilExist(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetMobileSettings().ElementToExistTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementExists(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<IWebDriver, bool> ElementExists<TBy>(TDriver searchContext, TBy by)
