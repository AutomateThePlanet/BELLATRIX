...
{
   public abstract class BaseUntil<TDriver, TDriverElement>
       where TDriver : AppiumDriver<TDriverElement>
       where TDriverElement : AppiumWebElement
    {
        protected BaseUntil(int? timeoutInterval = null, int? sleepInterval = null)
        {
            WrappedWebDriver = ServicesCollection.Current.Resolve<TDriver>();
            TimeoutInterval = timeoutInterval;
            SleepInterval = sleepInterval ?? ConfigurationService.Instance.GetMobileSettings().SleepInterval;
        }
        ...
        protected void WaitUntil(Func<TDriver, TDriverElement> waitCondition, int? timeout, int? sleepInterval)
        {
            if (timeout != null && sleepInterval != null)
            {
                var timeoutTimeSpan = TimeSpan.FromSeconds((int)timeout);
        ...
        protected TDriverElement FindElement<TBy>(TDriver searchContext, TBy by)
            where TBy : By<TDriver, TDriverElement>
        {
