...
{
    public class UntilBeVisible<TDriver, TDriverElement> : BaseUntil<TDriver, TDriverElement>
       where TDriver : AppiumDriver<TDriverElement>
       where TDriverElement : AppiumWebElement
    {
        public UntilBeVisible(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetMobileSettings().ElementToBeVisibleTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementIsVisible(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<TDriver, bool> ElementIsVisible<TBy>(TDriver searchContext, TBy by)
