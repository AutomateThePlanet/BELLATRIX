...
{
    public partial class Element<TDriver, TDriverElement>
    {
        ...
        protected Func<TSecond, TReturn> InitializeFunction<TElement, TSecond, TReturn>(
            TElement element,
            Func<TElement, TSecond, TReturn> globalAction,
            Func<TElement, TSecond, TReturn> localAction,
            Func<TElement, TSecond, TReturn> defaultAction)
            where TElement : Element<TDriver, TDriverElement>
        {
            if (localAction != null)
            {
                return (s1) => localAction(element, s1);
            }
            else if (globalAction != null)
            {
                return (s1) => globalAction(element, s1);
            }
            else
            {
                return (s1) => defaultAction(element, s1);
            }
        }

        protected Action InitializeAction<TElement>(TElement element, Action<TElement> globalAction, Action<TElement> defaultAction)
            where TElement : Element<TDriver, TDriverElement> => globalAction != null ? (Action)(() => globalAction(element)) : (() => defaultAction(element));

        protected Func<TReturn> InitializeAction<TElement, TReturn>(
            TElement element,
            Func<TElement, TReturn> globalAction,
            Func<TElement, TReturn> defaultAction)
            where TElement : Element<TDriver, TDriverElement> => globalAction != null ? (Func<TReturn>)(() => globalAction(element)) : (() => defaultAction(element));
    }
}
