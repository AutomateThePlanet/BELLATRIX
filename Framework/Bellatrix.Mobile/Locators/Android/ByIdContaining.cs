...
{
    public class ByIdContaining : By<AndroidDriver<AndroidElement>, AndroidElement>
    {
        private readonly string _locatorValue;

        public ByIdContaining(string name)
            : base(name) => _locatorValue = $"new UiScrollable(new UiSelector()).scrollIntoView(new UiSelector().resourceIdMatches(\".*{Value}.*\"));";
