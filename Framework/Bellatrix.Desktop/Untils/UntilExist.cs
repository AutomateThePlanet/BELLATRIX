...
namespace Bellatrix.Desktop.Untils
{
    public class UntilExist : BaseUntil
    {
        public UntilExist(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetDesktopSettings().ElementToExistTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementExists(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<IWebDriver, bool> ElementExists<TBy>(WindowsDriver<WindowsElement> searchContext, TBy by)
