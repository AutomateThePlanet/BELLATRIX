...
namespace Bellatrix.Desktop.Untils
{
    public class UntilBeVisible : BaseUntil
    {
        public UntilBeVisible(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetDesktopSettings().ElementToBeVisibleTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementIsVisible(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<IWebDriver, bool> ElementIsVisible<TBy>(WindowsDriver<WindowsElement> searchContext, TBy by)
