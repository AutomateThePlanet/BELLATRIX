...
namespace Bellatrix.Desktop.Untils
{
    public class UntilNotBeVisible : BaseUntil
    {
        public UntilNotBeVisible(int? timeoutInterval = null, int? sleepInterval = null)
            : base(timeoutInterval, sleepInterval) => TimeoutInterval = timeoutInterval ?? ConfigurationService.Instance.GetDesktopSettings().ElementNotToBeVisibleTimeout;

        public override void WaitUntil<TBy>(TBy by) => WaitUntil(ElementIsInvisible(WrappedWebDriver, by), TimeoutInterval, SleepInterval);

        private Func<IWebDriver, bool> ElementIsInvisible<TBy>(WindowsDriver<WindowsElement> searchContext, TBy by)
